...
async fn search_by_params(
	...
	(None,None,None) => query_as!(Name,
			    ...
			    .fetch_all(&state.db)
			    .await?,
	(None,None,Some(sheet_name)) => query_as!(Name,
				r#"
				SELECT id,sheet_name as the_name
			    ...
			    .fetch_all(&state.db)
			    .await?,
	(Some(begin),None,None) => query_as!(Name,
				r#"
				SELECT id,sheet_name as the_name
			    ...
			    .fetch_all(&state.db)
			    .await?,
	(Some(begin),None,Some(sheet_name)) => query_as!(Name,
				r#"
				SELECT id,sheet_name as the_name
			    ...
			    .fetch_all(&state.db)
			    .await?,
	(None,Some(end),None) => query_as!(Name,
				r#"
				SELECT id,sheet_name as the_name
				FROM sheets WHERE type_name = $1 AND insert_date < $2
				OFFSET $3 LIMIT 5"#,
			    ...
			    .fetch_all(&state.db)
			    .await?,
	(None,Some(end),Some(sheet_name)) => query_as!(Name,
				r#"
				SELECT id,sheet_name as the_name
				FROM sheets WHERE type_name = $1 AND
				(insert_date < $2 AND sheet_name LIKE $3)
			    ...
			    .fetch_all(&state.db)
			    .await?,
	(Some(begin),Some(end),None) => query_as!(Name,
				r#"
				SELECT id,sheet_name as the_name
			    ...
			    .fetch_all(&state.db)
			    .await?,
	(Some(begin),Some(end),Some(sheet_name)) => query_as!(Name,
				r#"
				SELECT id,sheet_name as the_name
				FROM sheets WHERE type_name = $1 AND
...
async fn save_sheet(
	...
	VALUES($1,$2,$3,$4)"#,
	id,sheet_name,type_name,insert_date,
    )
    .execute(&state.db)
    .await?;
    for row in rows {
